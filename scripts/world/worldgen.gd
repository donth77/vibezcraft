...
# Entry points added to WorldgenNative after it first shipped; each call
# site checks has_method and falls back to GDScript without them.
const _NATIVE_OPTIONAL_METHODS: Array[String] = [
	"fill_chunk_3d",
	"apply_surface_layer_3d",
	"settle_generated_gravity",
...
static func enable_native() -> bool:
	...
	_warn_if_native_is_stale()
	return _native_worldgen != null


# The GDScript side can run ahead of the compiled library: pulling new
# scripts without re-running scons leaves an older one that lacks newer
# entry points. Each call site falls back to GDScript on its own — which
# is silent, and slow (the stale library in issue #10 cost ~60 ms a chunk
# in the GDScript floating-terrain pass) — so say so once, at startup.
static func _warn_if_native_is_stale() -> void:
	if _native_worldgen == null:
		return
	var missing: PackedStringArray = []
	for method: String in _NATIVE_OPTIONAL_METHODS:
		if not _native_worldgen.has_method(method):
			missing.append(method)
	if not missing.is_empty():
		var names: String = ", ".join(missing)
		push_warning("Worldgen: WorldgenNative lacks %s — rebuild it with scons" % names)
...
static func generate_chunk(chunk_x: int, chunk_z: int) -> Chunk:
	...
	if terrain_3d_enabled:
		...
		if _native_worldgen != null and _native_worldgen.has_method("fill_chunk_3d"):
			...
			chunk.max_y = _top_non_air_layer(chunk.blocks)
...
# Highest y holding anything but air: the top layer whose 256 cells are
# not all AIR, which is the tallest column's top. Counted a layer at a
# time from the sky down so the per-cell work is a C++ count; walking
# every column down in GDScript cost ~5 ms a chunk.
static func _top_non_air_layer(blocks: PackedByteArray) -> int:
	const LAYER: int = Chunk.SIZE_X * Chunk.SIZE_Z
	for y: int in range(Chunk.SIZE_Y - 1, 0, -1):
		if blocks.slice(y * LAYER, (y + 1) * LAYER).count(Blocks.AIR) != LAYER:
			return y
	return 0
...
#
# The search dispatches to C++ (_floating_terrain_indices): the GDScript
# reference measured 60-90 ms per chunk in field logs, as much as the
# rest of generation combined.
...
static func _apply_cold_biome_overlay(chunk: Chunk, chunk_x: int, chunk_z: int) -> void:
	# All 256 column biomes in one native call when the extension has it:
	# Worldgen3D.biome_at samples three octave noises per column in
	# GDScript, ~5 ms a chunk mostly spent learning a column is not cold.
	var biomes := PackedByteArray()
	if _native_worldgen != null and _native_worldgen.has_method("biome_grid"):
		biomes = _native_worldgen.call("biome_grid", chunk_x, chunk_z)
	var native_biomes: bool = biomes.size() == Chunk.SIZE_X * Chunk.SIZE_Z
	for x in range(Chunk.SIZE_X):
		for z in range(Chunk.SIZE_Z):
			var world_x: int = chunk_x * Chunk.SIZE_X + x
			var world_z: int = chunk_z * Chunk.SIZE_Z + z
			var biome_id: int = (
				biomes[z * Chunk.SIZE_X + x]
				if native_biomes
				else Worldgen3D.biome_at(float(world_x), float(world_z))
			)
...
static func _post_process_native_caves(chunk: Chunk) -> void:
	...
	# zeroing the whole array is the same write at C++ speed.
	var meta := chunk.block_meta
	meta.fill(0)
	chunk.block_meta = meta
	# The flags are whole-chunk scans done as C++ searches. This pass
	# used to test every cell in GDScript — needs_gdscript_mesher's
	# branch chain 32,768 times a chunk, since a freshly carved chunk has
	# no non-cube cell to stop the scan early — and that was ~40 ms, most
	# of what generating a chunk cost (the carve itself is ~0.2 ms).
	if not chunk.has_non_cube_blocks:
		chunk.has_non_cube_blocks = Blocks.any_needs_gdscript_mesher(blocks)
	if not chunk.has_water_cells:
		chunk.has_water_cells = (blocks.has(Blocks.WATER_STILL) or blocks.has(Blocks.WATER_FLOWING))
