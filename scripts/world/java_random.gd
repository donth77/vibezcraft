...
func next_long() -> int:
	# OpenJDK: `((long)next(32) << 32) + next(32)`. BOTH terms are ints
	# and both widen to long with sign extension before the addition, so
	# a negative low word SUBTRACTS. Verified against the JDK itself by
	# tests/test_alpha_source_oracle.gd.
	var high: int = _to_signed_32(next(32))
	var low: int = _to_signed_32(next(32))
	return (high << 32) + low


# The pre-2026-08 reading of nextLong(), which treated the low word as
# unsigned and therefore ran 2^32 high whenever that word was negative.
#
# Kept — and still used by worldgen_caves.gd — because Alpha-parity is not
# the only contract that matters here: every Overworld world ever
# generated by this project has its caves carved from this sequence, and
# switching to the correct one would re-carve unvisited chunks of existing
# saves into different shapes than their already-persisted neighbours.
#
# The Overworld cave generator is therefore a KNOWN deviation from vanilla
# Alpha, documented in docs/nether-alpha-1.2.6-implementation-plan.md's
# Batch 0 handoff. New code — including everything in the Nether — must
# call next_long(). Do not introduce further callers of this one.
func next_long_legacy_unsigned_low() -> int:
	var high: int = _to_signed_32(next(32))
	var low: int = next(32)
	return (high << 32) + low
