...
# Suppression flag — true while LoadingScreen is up so the player's
# initial fall + chunk-manager startup doesn't trigger splash/step/etc.
# ChunkManager flips this after the spiral-load completes (same place
# Music.start_music is invoked).
...
func play_step(block_id: int) -> void:
	if not Game.sfx_enabled or not ready_to_play:
		return
